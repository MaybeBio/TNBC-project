...
# 1 vs 1 analysis 再取DEG交集，弃用
前面的都是1 vs 3直接全部rep比对的数据，这里使用的就是1 vs 1的模板
下面的分析都是1vs1的DEG取交集分析用的，但是我们上面就是1vs3全部rep都用上，所以下面的代码就不执行了
```{r}
#下面是HCC70的DEG分析
# Settings
fileNameOut1   <- file.path(dir_results, "edgeR_HMEC_vs_HCC70.txt") 
counts <- read_tsv(fileNameIn1)
counts <- counts %>% dplyr::select("gene_id", "symbol",starts_with("HMEC"),starts_with("HCC70")) 
sample_annotation <- data.frame(Sample = colnames(counts)[grepl("rep", colnames(counts))],
                                Group  = ifelse(grepl("HMEC", colnames(counts)[grepl("rep", colnames(counts))]), "HMEC", "HCC70"))

TPM <- read_tsv(fileNameIn2)
TPM <- TPM %>% dplyr::select("gene_id",starts_with("HMEC"),starts_with("HCC70"))
TPM <- TPM %>% dplyr::mutate(log2Average.HMEC = rowMeans(TPM %>% dplyr::select(starts_with("HMEC"))), .before = starts_with("HMEC")) %>%
  dplyr::mutate(log2Average.HCC70 = rowMeans(TPM %>% dplyr::select(starts_with("HCC70"))), .before = starts_with("HMEC"))
...
edgeR.dgelist = DGEList(counts = counts_subset[, 2:ncol(counts_subset)], genes = counts_subset$Geneid, group = Group)
keep <- filterByExpr(edgeR.dgelist)
edgeR.dgelist <- edgeR.dgelist[keep, , keep.lib.sizes=FALSE] 
edgeR.dgelist = calcNormFactors((edgeR.dgelist), method = "TMM")
edgeR.dgelist = estimateDisp(edgeR.dgelist, design, robust=T) 
...
#然后这个文件是可以直接进行DEG分析的,
sign_genes_HCC70 <-subset(res.full_to_save, FDR < 0.05 & (logFC > 1 | logFC < -1))   #7334
write_tsv(sign_genes_HCC70 , file = "edgeR_HMEC_vs_HCC70_FDR0.05_logFC1_DEG.txt")      

sign_genes_HCC70_1 <-subset(res.full_to_save, FDR < 0.005 & (logFC > 4 | logFC < -4))
write_tsv(sign_genes_HCC70_1 , file = "edgeR_HMEC_vs_HCC70_FDR0.005_logFC4_DEG.txt") #2112,接近2k，用这个数据作为后续分析的基础



#手头上的3个DEG文件：
#sign_genes_HCC70_1：HCC70 FDR=0.005 logFC=4
#sign_genes_MB231_1：MB231 同上阈值
#sign_genes_BT549_1:BT549 同上阈值
#分别提取出up以及down的gene：
#下面有2种方法：1就是直接对DEG取交集，但是这些gene对于HMEC究竟是上调还是下调其实不确定的，也就是说是无法确定的
#2就是直接分别提取出各自up以及down的gene，然后直接对gene取一个交集

#HCC70中上下调
HCC70_up_DEGs <- subset(sign_genes_HCC70_1, logFC > 4) #906
#subset(sign_genes_HCC70, logFC > 1) 3586
HCC70_down_DEGs <- subset(sign_genes_HCC70_1, logFC < -4) #1206
#subset(sign_genes_HCC70, logFC < -1) 3748

#MB231中上下调
MB231_up_DEGs <- subset(sign_genes_MB231_1, logFC > 4) #896
#subset(sign_genes_MB231, logFC > 1) 3396
MB231_down_DEGs <- subset(sign_genes_MB231_1, logFC < -4) #1522
#subset(sign_genes_MB231, logFC < -1)  4120

#BT549中上下调
BT549_up_DEGs <- subset(sign_genes_BT549_1, logFC > 4) #647
#subset(sign_genes_BT549, logFC > 1)
BT549_down_DEGs <- subset(sign_genes_BT549_1, logFC < -4) #1209
#subset(sign_genes_BT549, logFC < -1)

TNBC_up_DEG<- Reduce(intersect, list(HCC70_up_DEGs$genes, MB231_up_DEGs$genes, BT549_up_DEGs$genes)) #71,还是要少了，或者是上游之后得到
#Reduce(intersect, list(subset(sign_genes_MB231, logFC > 1)$genes, subset(sign_genes_MB231, logFC > 1)$genes, subset(sign_genes_BT549, logFC > 1)$genes)) 1124
TNBC_dn_DEG<-Reduce(intersect, list(HCC70_down_DEGs$genes, MB231_down_DEGs$genes, BT549_down_DEGs$genes)) #343
#Reduce(intersect, list(subset(sign_genes_HCC70, logFC < -1)$genes, subset(sign_genes_MB231, logFC < -1)$genes, subset(sign_genes_BT549, logFC < -1)$genes)) 1373

  
#如果是3个钟至少有2个出现，即3中取》2
# 遍历每个基因
common_genes <- c()
for (gene in unique(c(HCC70_up_DEGs$genes, MB231_up_DEGs$genes, BT549_up_DEGs$genes))) {
  # 检查基因在每个对象中的表达状态
  in_HCC70 <- gene %in% HCC70_up_DEGs$genes
  in_MB231 <- gene %in% MB231_up_DEGs$genes
  in_BT549 <- gene %in% BT549_up_DEGs$genes
  # 检查基因在至少两个对象中是上调的
  if ((in_HCC70 & in_MB231) || (in_HCC70 & in_BT549) || (in_MB231 & in_BT549)) {
...
common_genes <- c()
for (gene in unique(c(HCC70_down_DEGs$genes, MB231_down_DEGs$genes, BT549_down_DEGs$genes))) {
  # 检查基因在每个对象中的表达状态
  in_HCC70 <- gene %in% HCC70_down_DEGs$genes
  in_MB231 <- gene %in% MB231_down_DEGs$genes
  in_BT549 <- gene %in% BT549_down_DEGs$genes
  # 检查基因在至少两个对象中是上调的
  if ((in_HCC70 & in_MB231) || (in_HCC70 & in_BT549) || (in_MB231 & in_BT549)) {
    common_genes <- c(common_genes, gene)
  }
