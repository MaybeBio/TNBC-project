...
for rep in ${myrep[@]};do
  ...
  bedtools intersect -a templeft.bed -b ${enhancerdir}/${rep}_SE_sort.bed ${enhancerdir}/${rep}_TE_sort.bed ${promoter} -wa -wb -loj -sortout -names SE TE P > ${rep}_${rep}_leftanchor.bed
rm templeft.bed
  #下面是对同一loop中的HMEC而言
  ...
  bedtools intersect -a templeft.bed -b ${enhancerdir}/HMEC_SE_sort.bed ${enhancerdir}/HMEC_TE_sort.bed ${promoter} -wa -wb -loj -sortout -names SE TE P > ${rep}_HMEC_leftanchor.bed
rm templeft.bed

  #下面是对右anchor而言
  cut -f 4-6,12 ${loopdir}/${rep}_preprocessing_any/loops_${rep}_unique_10000_withid.bedpe | tail -n+2 > tempright.bed
  bedtools intersect -a tempright.bed -b ${enhancerdir}/${rep}_SE_sort.bed ${enhancerdir}/${rep}_TE_sort.bed ${promoter} -wa -wb -loj -sortout -names SE TE P > ${rep}_${rep}_rightanchor.bed
rm tempright.bed
  #下面是对同一loop中的HMEC而言
  cut -f 4-6,12 ${loopdir}/${rep}_preprocessing_any/loops_HMEC_unique_10000_withid.bedpe | tail -n+2 > tempright.bed
  bedtools intersect -a tempright.bed -b ${enhancerdir}/HMEC_SE_sort.bed ${enhancerdir}/HMEC_TE_sort.bed ${promoter} -wa -wb -loj -sortout -names SE TE P > ${rep}_HMEC_rightanchor.bed
rm tempright.bed
