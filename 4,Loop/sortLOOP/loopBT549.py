...
with open("BT549_BT549_leftanchor.bed", "r") as left_file:
...
with open("BT549_BT549_rightanchor.bed", "r") as right_file:
...
# 输出组合后的数据到文件
with open("BT549_BT549_loop_annotation.bed", "w") as output_file:
...
with open("BT549_HMEC_leftanchor.bed", "r") as left_file:
    for line in left_file:
        parts = line.strip().split("\t")
        loop_id = parts[3]
        if loop_id not in left_anchor_data:
...
with open("BT549_HMEC_rightanchor.bed", "r") as right_file:
    for line in right_file:
        parts = line.strip().split("\t")
        loop_id = parts[3]
        if loop_id not in right_anchor_data:
...
# 输出组合后的数据到文件
with open("BT549_HMEC_loop_annotation.bed", "w") as output_file:
    for row in combined_data:
        output_file.write("\t".join(row) + "\n")
